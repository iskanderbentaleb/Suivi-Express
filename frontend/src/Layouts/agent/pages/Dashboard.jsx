...
import { dashboard } from '../../../services/api/agent/dashboard';
...
export default function Dashboard() {
  ...
  const stats = [
    { title: "Le total des commandes que j'ai suivies", icon: 'box', value: dashboardData.total_orders },
    { title: 'Mon Taux de Livraison', icon: 'percentage', value: dashboardData.delivery_rate },
  ];
  ...
  return (
    <Container size="xl" py="xl" px="md">
      ...
      {/* Stats Cards */}
      <SimpleGrid cols={{ base: 1, xs: 2, md: 2 }} spacing="lg">
        {loading
                ...
                <Card withBorder p="lg" radius="lg" key={stat.title} shadow="sm" className={classes.card}>
                  ...
                  <Group align="flex-end" gap="xs" mt={20}>
                    <Text className={classes.value} fz="xl" fw={800}>
                      {stat.title === 'Mon Taux de Livraison' ? (
                        <CountUp
                          end={stat.value}
                          suffix="%"
                          duration={2}
